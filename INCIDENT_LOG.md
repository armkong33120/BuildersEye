...
| 2026-08-23 20:35 | หน้า demo `debug_neural_network_diagram.html` แสดง "⚠ no credentials returned" ตลอด ถึงแม้ `ENABLE_TEST_CREDS=true` (production มี password ชุดเดียวกับ Azure) ทำให้ dropdown บัญชีทดสอบว่าง และผู้ชมเลือกเครดิตเพื่อ Auto-login ไม่ได้ | แก้ `loadCredentials()` ฝั่ง frontend ให้ unwrap รายชื่อจาก `body.users` (endpoint `/api/preview/credentials` คืน object `{enabled,password,users}` ไม่ใช่ array ตรงๆ) → dropdown คืนรายชื่อ + `✓ N test accounts` | Resolved |
