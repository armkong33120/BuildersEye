...
- **Test-only HTTP timeout (`scripts/test_helpers.mjs`)**: auth-gated test scripts import `TEST_HTTP_TIMEOUT_MS` from this shared helper instead of hardcoding `AbortSignal.timeout(...)`. Default `30000`, override with `TEST_HTTP_TIMEOUT_MS=60000` for slow backends (Neon login ~18 s). Test-only — never changes server/JWT/Neon/application timeouts.

## Result (2026-08-18, Neon authenticated verification — final gate)
Ran full suite against a **Neon-backed backend** (`ACCESS_DB_ADAPTER=neon`, live backend on :5199, `TEST_HTTP_TIMEOUT_MS=60000`).
- **Auth-gated suites — ALL PASS against Neon (0 timeouts, 0 errors):** Blocked Queries, Debug Auth, Session Refresh, Vector Query, SQL Query, Cache Hit, SQL Fallback, SQL Metadata+Evidence, SQL Evidence+History, Isolation API (live), RBAC Matrix. Plus Invalid Login. **No auth-gated suite was skipped** (auth configured + backend up).
- **Neon adapter** `scripts/test_access_neon_adapter.mjs`: **13/13** (schema, preload, seed, write/read, policy-version atomicity, version column, save policy, audit CRUD).
- **Regression unit suites — pass under their designed default (JSON) adapter** (these use a temp `ACCESS_DATA_DIR` and must NOT be forced onto the Neon adapter): org integrity 32/32, admin service 20/20, admin preview contract 48/48, isolation security 46/46, canonical policy 25/25, legacy shim parity 38/38, persistence restart 14/14 (one transient flake on a single early run, then 14/14 across 3 consecutive clean runs). verify:security 34/34, build OK, benchmark:dynamic 75/75 (leakage 0%).
- **Latency measured (Neon backend):** login **p50=17707 ms / p95=18029 ms**; authenticated API **p50=2 ms / p95=7 ms**; admin API + policy read **sub-10 ms** (Neon adapter in-memory read cache). **Root cause of ~18 s login confirmed by direct Neon timing**: login rewrites ALL `auth_sessions` on every call (`DELETE` all + sequential `INSERT` per session) — 60 sequential inserts = 5219 ms (~87 ms/insert); with 215 accumulated sessions that is ~18 s. NOT connection establishment (cold connect 834 ms), NOT cold pool (warm SELECT ~70 ms), NOT backend startup, NOT retry, NOT test harness. It is an O(n) session-write pattern in `server/authStore.js` (unrelated to the access adapter).
- **Correct adapter handling:** `npm test` must be run WITHOUT `ACCESS_DB_ADAPTER=neon` exported for the unit/regression suites (they isolate via temp `ACCESS_DATA_DIR`, which the Neon adapter ignores). Force Neon only for the live/auth-gated suites and `test_access_neon_adapter.mjs`.
