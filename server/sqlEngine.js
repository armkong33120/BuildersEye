...
Schema:
- employeeId (INT): the numeric primary key of the employee (e.g. 63 for EMP063)
- employeeCode (STRING): the employee code like "EMP063" — ALWAYS use employeeCode (NOT employeeId) when the user mentions an employee by code/name like EMP063
- employeeName (STRING)
- department (STRING): department names are stored in ENGLISH — map Thai department mentions to the English values exactly: Executive, Sales, Marketing, Design & Architecture, Engineering & Construction, Procurement & Warehouse, Customer Service & Warranty, Finance & Accounting, HR & Admin, IT, Legal, Office Support (e.g. "ฝ่ายจัดซื้อ" → 'Procurement & Warehouse', "วิศวกรรม" → 'Engineering & Construction', "ฝ่ายขาย" → 'Sales')
...
- fieldName (STRING): MUST BE ONE OF ["kpiScore", "okrScore", "formalWarning", "verbalWarning", "department", "position", "Cost_THB", "Cost_Per_Seat_THB", "Asset_Type", "Brand", "Model", "Status", "Ticket_Issue", "Description", "Resolution", "Priority", "Reported_Date", "Location", "License_Name", "Base_Salary", "Bonus_Months", "Increase_Percent", "Sick_Leave_Days", "Personal_Leave_Days", "Late_Arrivals", "Notes", "Reviewer_Type", "Comment", "Core_Skill", "Language_Score_IELTS", "Certification", "Skill_Level", "Last_Assessed", "Flight_Risk_Pct", "Impact_of_Loss", "Readiness", "Medical_THB", "Dental_THB", "Wellness_THB", "Travel_THB", "Entertainment_THB", "Office_Supplies_THB", "Category", "Amount_THB", "Date", "Approver", "Complaint_Type", "Details", "Mandate", "Culture_Fit_Score", "Interview_Score", "Buddy_Name", "eNPS", "Burnout_Risk", "Access_Zone", "Parking_Slot", "Last_Badge_Swipe", "Badge_Swipes_Week", "Fingerprint_Scans_Week", "Billable_Hours_Pct", "Admin_Hours_Pct", "Week", "Overtime_Hours", "Missing_Punch", "managerFeedback", "improvementPlan", "weakArea", "strongArea", "reviewPeriod", "followUpStatus", "performanceBand", "mistakeIssue", "recoveryAction", "individualOutcome", "contributionSummary", "conflictSummary", "resolutionSummary", "collaborationQuality", "relationshipType", "caseType", "severity", "rootCause", "actionTaken", "summary", "budgetTHB", "actualCostTHB", "customerId", "product_name", "unit_price_thb", "category", "revenue_thb", "profit_margin_pct", "deal_value_thb", "deal_name", "stage", "probability_pct", "amount_thb", "contract_value_thb", "account_manager_emp_id", "headcount_cost_thb", "cogs_thb", "operating_cost_thb", "net_profit_thb", "allocated_budget_thb"]
...
2a. **FREE-TEXT THAI DATA:** many events are stored as free Thai text in Notes / Description / summary / mistakeIssue / managerFeedback (NOT structured status values). Do NOT look for statuses like 'Loan Rejected' / 'Deposit Abandoned' — instead use content LIKE '%กู้%' / '%ทิ้งดาวน์%' / '%เทปูน%' etc. When the user asks for details/stories (อธิบาย, เล่า, รายละเอียด), SELECT the text fields (Notes, Description, summary) too — e.g. for OT questions SELECT both Overtime_Hours AND Notes.
